...
router.get('/team-insights', async (req, res) => {
  try {
    ...
    const teamSpreadPerformance = await allQuery<any>(
      `SELECT 
    ...
    const teamStats = teamSpreadPerformance.reduce((acc: any, team: any) => {
    ...
    const finalTeamStats = Object.values(teamStats)
      ...
      .filter((team: any) => team.total_games >= 2) // Only teams with 2+ games
      .sort((a: any, b: any) => b.spread_win_percentage - a.spread_win_percentage)
      .slice(0, 10); // Top 10
...
const CONFERENCE_MAPPING: { [key: string]: string } = {
  // SEC
...
router.get('/conference-insights', async (req, res) => {
  try {
    ...
    const conferenceStats: { [key: string]: {
      conference: string;
      total_games: number;
      spread_wins: number;
      spread_losses: number;
      spread_win_percentage: number;
      avg_spread_margin: number;
      spread_margins: number[];
    }} = {};
    ...
    const conferenceMatchups = games.filter((game: any) => {
...
router.get('/insights', async (req, res) => {
  try {
    ...
    const biggestUpsets = await allQuery<any>(
      `SELECT 
         g.home_team,
         g.away_team,
         g.spread,
