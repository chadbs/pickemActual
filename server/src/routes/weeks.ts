...
router.get('/', async (req, res) => {
  try {
    ...
    const weeks = await allQuery<Week>(
      `SELECT * FROM weeks ${whereClause} ORDER BY season_year DESC, week_number DESC`,
      params
    );
...
router.get('/current', async (req, res) => {
  try {
    const currentWeek = await getQuery<Week>(
      'SELECT * FROM weeks WHERE is_active = 1 ORDER BY season_year DESC, week_number DESC LIMIT 1'
    );
    ...
    if (!currentWeek) {
      ...
      const result = await runQuery(
        ...
        [weekNumber, year, deadline.toISOString(), true, 'active']
      );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const week = await getQuery<Week>('SELECT * FROM weeks WHERE id = ?', [id]);
...
router.get('/season/:year/week/:weekNumber', async (req, res) => {
  try {
    ...
    const week = await getQuery<Week>(
      'SELECT * FROM weeks WHERE season_year = ? AND week_number = ?',
      [year, weekNumber]
    );
...
router.get('/:id/summary', async (req, res) => {
  try {
    ...
    const games = await allQuery<any>(
      'SELECT * FROM games WHERE week_id = ? ORDER BY start_time ASC',
      [id]
    );
    
    // Get pick statistics
    const pickStats = await allQuery<any>(
      `SELECT 
         u.name,
       ...
       ORDER BY correct_picks DESC, u.name ASC`,
      [id]
    );
...
router.post('/', async (req, res) => {
  try {
    ...
    if (is_active) {
      await runQuery('UPDATE weeks SET is_active = 0');
    }
    
    const result = await runQuery(
      'INSERT INTO weeks (week_number, season_year, deadline, is_active, status) VALUES (?, ?, ?, ?, ?)',
      [week_number, season_year, deadline, is_active, req.body.status || 'upcoming']
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existingWeek = await getQuery<Week>('SELECT * FROM weeks WHERE id = ?', [id]);
    if (!existingWeek) {
      return res.status(404).json({ error: 'Week not found' });
    }
    ...
    if (is_active && is_active !== existingWeek.is_active) {
      await runQuery('UPDATE weeks SET is_active = 0 WHERE id != ?', [id]);
    }
    
    // Update week
    await runQuery(
      'UPDATE weeks SET week_number = ?, season_year = ?, deadline = ?, is_active = ?, status = ? WHERE id = ?',
      [
    ...
    const updatedWeek = await getQuery<Week>('SELECT * FROM weeks WHERE id = ?', [id]);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await runQuery('DELETE FROM picks WHERE game_id IN (SELECT id FROM games WHERE week_id = ?)', [id]);
    await runQuery('DELETE FROM weekly_scores WHERE week_id = ?', [id]);
    await runQuery('DELETE FROM games WHERE week_id = ?', [id]);
    await runQuery('DELETE FROM weeks WHERE id = ?', [id]);
...
router.post('/:id/activate', async (req, res) => {
  try {
    ...
    // Activate this week
    await runQuery('UPDATE weeks SET is_active = 1, status = ? WHERE id = ?', ['active', id]);
    
    const activatedWeek = await getQuery<Week>('SELECT * FROM weeks WHERE id = ?', [id]);
