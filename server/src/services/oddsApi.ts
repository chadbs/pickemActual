...
export const getNCAAFootballOdds = async (): Promise<OddsAPIGameResponse[]> => {
  try {
...
export const parseOddsData = (oddsData: OddsAPIGameResponse[]): ParsedOdds[] => {
  return oddsData.map(game => {
    ...
    let bookmaker = game.bookmakers.find(book => 
      preferredBooks.includes(book.key.toLowerCase())
    ) || game.bookmakers[0];
...
export const matchOddsToGames = (cfbdGames: any[], oddsData: ParsedOdds[]): any[] => {
...
export const checkAPIUsage = async (): Promise<{ remaining: number; used: number }> => {
  try {
    const oddsApi = getOddsApi();
