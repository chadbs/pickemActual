...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const currentWeek = await (0, database_1.getQuery)(`SELECT w.*, COUNT(g.id) as game_count, COUNT(p.id) as pick_count
       FROM weeks w
       ...
       GROUP BY w.id`);
        // Get recent activity
        const recentActivity = await (0, database_1.allQuery)(`SELECT 'pick' as type, u.name, p.created_at, g.home_team, g.away_team, p.selected_team
       FROM picks p
...
router.post('/fetch-games-for-week', async (req, res) => {
    try {
        ...
        if (week_id) {
            ...
            weekData = await (0, database_1.getQuery)('SELECT * FROM weeks WHERE week_number = ? AND season_year = ?', [week_number, year]);
        }
        if (!weekData) {
        ...
        const cfbdGames = await (0, cfbDataApi_1.getTopGamesForWeek)(year, week_number);
        // Fetch odds data
        let oddsData = [];
        try {
...
router.post('/fetch-all-games', async (req, res) => {
    try {
        await (0, scheduler_1.fetchAllSeasonGames)();
...
router.delete('/seasons/:year', async (req, res) => {
    try {
        ...
        await (0, database_1.runQuery)(`DELETE FROM picks WHERE game_id IN (
      SELECT g.id FROM games g 
      JOIN weeks w ON g.week_id = w.id 
      WHERE w.season_year = ?
    )`, [yearNum]);
        // Delete weekly scores
        await (0, database_1.runQuery)(`DELETE FROM weekly_scores WHERE week_id IN (
      SELECT id FROM weeks WHERE season_year = ?
    )`, [yearNum]);
        // Delete season standings
        await (0, database_1.runQuery)('DELETE FROM season_standings WHERE season_year = ?', [yearNum]);
        // Delete games
        await (0, database_1.runQuery)(`DELETE FROM games WHERE week_id IN (
      SELECT id FROM weeks WHERE season_year = ?
    )`, [yearNum]);
        // Delete weeks
        await (0, database_1.runQuery)('DELETE FROM weeks WHERE season_year = ?', [yearNum]);
    ...
    try {
        ...
        for (const game of gamesWithOdds.slice(0, 8)) { // Max 8 games
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            const newGame = await (0, database_1.getQuery)('SELECT * FROM games WHERE id = ?', [result.lastID]);
...
router.post('/recalculate', async (req, res) => {
    try {
        ...
        const weeks = await (0, database_1.allQuery)('SELECT * FROM weeks ORDER BY season_year, week_number');
        for (const week of weeks) {
            const users = await (0, database_1.allQuery)('SELECT * FROM users');
            for (const user of users) {
                const userPicks = await (0, database_1.allQuery)(`SELECT p.*, g.status, g.spread_winner 
           FROM picks p 
           JOIN games g ON p.game_id = g.id 
           WHERE p.user_id = ? AND g.week_id = ?`, [user.id, week.id]);
             ...
             VALUES (?, ?, ?, ?, ?)`, [user.id, week.id, correctPicks, totalPicks, percentage]);
                }
            }
        }
        // Recalculate season standings
        const seasons = await (0, database_1.allQuery)('SELECT DISTINCT season_year FROM weeks ORDER BY season_year');
        for (const season of seasons) {
            const users = await (0, database_1.allQuery)('SELECT * FROM users');
            for (const user of users) {
                const seasonStats = await (0, database_1.getQuery)(`SELECT 
             SUM(ws.correct_picks) as total_correct,
             SUM(ws.total_picks) as total_picks
           FROM weekly_scores ws
           JOIN weeks w ON ws.week_id = w.id
           WHERE ws.user_id = ? AND w.season_year = ?`, [user.id, season.season_year]);
                if (seasonStats && seasonStats.total_picks > 0) {
                    const seasonPercentage = (seasonStats.total_correct / seasonStats.total_picks) * 100;
                    await (0, database_1.runQuery)(`INSERT OR REPLACE INTO season_standings 
             (user_id, season_year, total_correct, total_picks, season_percentage)
             VALUES (?, ?, ?, ?, ?)`, [user.id, season.season_year, seasonStats.total_correct, seasonStats.total_picks, seasonPercentage]);
                }
            }
        }
...
router.get('/api-usage', async (req, res) => {
    try {
        const oddsUsage = await (0, oddsApi_1.checkAPIUsage)();
        res.json({
            odds_api: {
    ...
    try {
        ...
        for (let weekNum = 1; weekNum <= 15; weekNum++) {
            ...
            if (!existingWeek) {
                ...
                const result = await (0, database_1.runQuery)('INSERT INTO weeks (week_number, season_year, deadline, is_active, status) VALUES (?, ?, ?, ?, ?)', [weekNum, seasonYear, deadline.toISOString(), isActive, status]);
                createdWeeks.push({
                    id: result.lastID,
                    week_number: weekNum,
    ...
    try {
        ...
        switch (action) {
            case 'cleanup_old_data':
                ...
                await (0, database_1.runQuery)('DELETE FROM picks WHERE game_id IN (SELECT g.id FROM games g JOIN weeks w ON g.week_id = w.id WHERE w.season_year < ?)', [cutoffDate.getFullYear()]);
                await (0, database_1.runQuery)('DELETE FROM weekly_scores WHERE week_id IN (SELECT id FROM weeks WHERE season_year < ?)', [cutoffDate.getFullYear()]);
                await (0, database_1.runQuery)('DELETE FROM games WHERE week_id IN (SELECT id FROM weeks WHERE season_year < ?)', [cutoffDate.getFullYear()]);
...
router.get('/export/:type', async (req, res) => {
    try {
        ...
        switch (type) {
            case 'picks':
                data = await (0, database_1.allQuery)(`SELECT p.*, u.name as user_name, g.home_team, g.away_team, g.spread, w.week_number, w.season_year
           FROM picks p
           JOIN users u ON p.user_id = u.id
           JOIN games g ON p.game_id = g.id
           JOIN weeks w ON g.week_id = w.id
           ${year ? 'WHERE w.season_year = ?' : ''}
           ORDER BY w.season_year, w.week_number, g.start_time, u.name`, year ? [year] : []);
                break;
            case 'leaderboard':
                data = await (0, database_1.allQuery)(`SELECT u.name, ws.*, w.week_number, w.season_year
           FROM users u
           JOIN weekly_scores ws ON u.id = ws.user_id
           JOIN weeks w ON ws.week_id = w.id
           ${year ? 'WHERE w.season_year = ?' : ''}
           ORDER BY w.season_year, w.week_number, ws.percentage DESC`, year ? [year] : []);
                break;
            case 'games':
                data = await (0, database_1.allQuery)(`SELECT g.*, w.week_number, w.season_year
           FROM games g
           JOIN weeks w ON g.week_id = w.id
           ${year ? 'WHERE w.season_year = ?' : ''}
           ORDER BY w.season_year, w.week_number, g.start_time`, year ? [year] : []);
                break;
            default:
