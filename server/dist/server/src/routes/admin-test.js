...
router.post('/cleanup-2024', async (req, res) => {
    try {
        ...
        const week2024Count = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM weeks WHERE season_year = 2024');
        const game2024Count = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM games WHERE week_id IN (SELECT id FROM weeks WHERE season_year = 2024)');
        const pick2024Count = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM picks WHERE game_id IN (SELECT g.id FROM games g JOIN weeks w ON g.week_id = w.id WHERE w.season_year = 2024)');
        const weeklyScore2024Count = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM weekly_scores WHERE week_id IN (SELECT id FROM weeks WHERE season_year = 2024)');
        ...
        await (0, database_1.runQuery)('DELETE FROM picks WHERE game_id IN (SELECT g.id FROM games g JOIN weeks w ON g.week_id = w.id WHERE w.season_year = 2024)');
        console.log(`🗑️ Deleted ${pick2024Count?.count || 0} picks from 2024`);
        // 2. Delete weekly scores (references weeks) 
        await (0, database_1.runQuery)('DELETE FROM weekly_scores WHERE week_id IN (SELECT id FROM weeks WHERE season_year = 2024)');
        console.log(`🗑️ Deleted ${weeklyScore2024Count?.count || 0} weekly scores from 2024`);
        // 3. Delete games (references weeks)
        await (0, database_1.runQuery)('DELETE FROM games WHERE week_id IN (SELECT id FROM weeks WHERE season_year = 2024)');
        console.log(`🗑️ Deleted ${game2024Count?.count || 0} games from 2024`);
        // 4. Finally delete weeks
        await (0, database_1.runQuery)('DELETE FROM weeks WHERE season_year = 2024');
        console.log(`🗑️ Deleted ${week2024Count?.count || 0} weeks from 2024`);
        // Verify cleanup
        const remainingWeeks = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM weeks WHERE season_year = 2024');
        const remainingGames = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM games WHERE week_id IN (SELECT id FROM weeks WHERE season_year = 2024)');
...
router.post('/update-active-week', async (req, res) => {
    try {
        ...
        const schedulerModule = await Promise.resolve().then(() => __importStar(require('../services/scheduler')));
        ...
        const currentActive = await (0, database_1.getQuery)('SELECT * FROM weeks WHERE is_active = 1 LIMIT 1');
        ...
        const newActive = await (0, database_1.getQuery)('SELECT * FROM weeks WHERE is_active = 1 LIMIT 1');
...
router.delete('/delete-user/:user_id', async (req, res) => {
    try {
        ...
        const user = await (0, database_1.getQuery)('SELECT * FROM users WHERE id = ? LIMIT 1', [user_id]);
        if (!user) {
        ...
        await (0, database_1.runQuery)('BEGIN TRANSACTION');
        try {
            // Delete user's picks first (foreign key constraint)
            const picksDeleted = await (0, database_1.runQuery)('DELETE FROM picks WHERE user_id = ?', [user_id]);
            // Delete the user
            const userDeleted = await (0, database_1.runQuery)('DELETE FROM users WHERE id = ?', [user_id]);
            // Commit transaction
            await (0, database_1.runQuery)('COMMIT');
            ...
            await (0, database_1.runQuery)('ROLLBACK');
            throw error;
        }
    }
    catch (error) {
...
router.post('/cleanup-orphaned-data', async (req, res) => {
    try {
        ...
        const currentUsers = await (0, database_1.allQuery)('SELECT id FROM users');
        ...
        const orphanedPicks = await (0, database_1.allQuery)(`SELECT DISTINCT user_id, COUNT(*) as pick_count
       FROM picks 
       WHERE user_id NOT IN (${currentUserIds.map(() => '?').join(',')})
       GROUP BY user_id`, currentUserIds);
        // Find orphaned weekly scores
        const orphanedWeeklyScores = await (0, database_1.allQuery)(`SELECT DISTINCT user_id, COUNT(*) as score_count
       FROM weekly_scores 
       WHERE user_id NOT IN (${currentUserIds.map(() => '?').join(',')})
       GROUP BY user_id`, currentUserIds);
        // Find orphaned season standings
        const orphanedSeasonStandings = await (0, database_1.allQuery)(`SELECT DISTINCT user_id, COUNT(*) as standing_count
       ...
       GROUP BY user_id`, currentUserIds);
        ...
        if (orphanedPicks.length > 0) {
            ...
            const pickResult = await (0, database_1.runQuery)(`DELETE FROM picks WHERE user_id NOT IN (${currentUserIds.map(() => '?').join(',')})`, currentUserIds);
        ...
        if (orphanedWeeklyScores.length > 0) {
            const weeklyResult = await (0, database_1.runQuery)(`DELETE FROM weekly_scores WHERE user_id NOT IN (${currentUserIds.map(() => '?').join(',')})`, currentUserIds);
        ...
        if (orphanedSeasonStandings.length > 0) {
            const seasonResult = await (0, database_1.runQuery)(`DELETE FROM season_standings WHERE user_id NOT IN (${currentUserIds.map(() => '?').join(',')})`, currentUserIds);
...
router.get('/preview-orphaned-data', async (req, res) => {
    try {
        // Get all current valid user IDs
        const currentUsers = await (0, database_1.allQuery)('SELECT id, name FROM users ORDER BY id');
        const currentUserIds = currentUsers.map(u => u.id);
        ...
        const orphanedData = await (0, database_1.allQuery)(`SELECT 
        'picks' as table_name,
        p.user_id,
       ...
       ORDER BY user_id, table_name`, [...currentUserIds, ...currentUserIds, ...currentUserIds]);
        ...
        res.json({
            ...
            summary: {
                ...
                total_orphaned_records: Object.values(groupedOrphans).reduce((sum, user) => sum + user.total_records, 0)
            }
        });
    }
    catch (error) {
