...
router.get('/', async (req, res) => {
    try {
       ...
       ${whereClause}
       ORDER BY w.season_year DESC, w.week_number DESC, g.start_time ASC, u.name ASC`, params);
        res.json(picks);
    }
    catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const pick = await (0, database_1.getQuery)(`SELECT p.*, u.name as user_name,
              g.home_team, g.away_team, g.spread, g.start_time, g.status,
              g.home_score, g.away_score, g.spread_winner,
...
router.post('/', async (req, res) => {
    try {
        ...
        // Get or create user
        let user = await (0, database_1.getQuery)('SELECT * FROM users WHERE name = ?', [user_name.trim()]);
        if (!user) {
            const userResult = await (0, database_1.runQuery)('INSERT INTO users (name) VALUES (?)', [user_name.trim()]);
            user = {
        ...
        const game = await (0, database_1.getQuery)(`SELECT g.*, w.deadline, w.status as week_status
       FROM games g
       JOIN weeks w ON g.week_id = w.id
       WHERE g.id = ?`, [game_id]);
        if (!game) {
        ...
        const existingPick = await (0, database_1.getQuery)('SELECT * FROM picks WHERE user_id = ? AND game_id = ?', [user.id, game_id]);
        if (existingPick) {
            // Update existing pick
            await (0, database_1.runQuery)('UPDATE picks SET selected_team = ?, confidence_points = ?, updated_at = ? WHERE id = ?', [selected_team, confidence_points, new Date().toISOString(), existingPick.id]);
            const updatedPick = await (0, database_1.getQuery)(`SELECT p.*, u.name as user_name,
                g.home_team, g.away_team, g.spread, g.start_time, g.status
         FROM picks p
         JOIN users u ON p.user_id = u.id
         JOIN games g ON p.game_id = g.id
         WHERE p.id = ?`, [existingPick.id]);
            return res.json(updatedPick);
        }
        else {
            // Create new pick
            const result = await (0, database_1.runQuery)('INSERT INTO picks (user_id, game_id, selected_team, confidence_points) VALUES (?, ?, ?, ?)', [user.id, game_id, selected_team, confidence_points]);
            const newPick = await (0, database_1.getQuery)(`SELECT p.*, u.name as user_name,
                g.home_team, g.away_team, g.spread, g.start_time, g.status
         FROM picks p
         JOIN users u ON p.user_id = u.id
         JOIN games g ON p.game_id = g.id
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existingPick = await (0, database_1.getQuery)(`SELECT p.*, g.*, w.deadline, w.status as week_status
       FROM picks p
       JOIN games g ON p.game_id = g.id
       JOIN weeks w ON g.week_id = w.id
       WHERE p.id = ?`, [id]);
        ...
        // Update pick
        await (0, database_1.runQuery)('UPDATE picks SET selected_team = ?, confidence_points = ?, updated_at = ? WHERE id = ?', [
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const existingPick = await (0, database_1.getQuery)(`SELECT p.*, w.deadline, w.status as week_status
       FROM picks p
       JOIN games g ON p.game_id = g.id
       JOIN weeks w ON g.week_id = w.id
       WHERE p.id = ?`, [id]);
        ...
        // Delete pick
        await (0, database_1.runQuery)('DELETE FROM picks WHERE id = ?', [id]);
